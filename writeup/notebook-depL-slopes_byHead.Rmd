...
#  agg3 = agg3[(1:min(nrow(agg3),30)),]
  plot = ggplot(agg3, aes(x=Head,y=Mean_Slope_DepLength, color=FileName, fill=Real_DH_Mean)) +
    geom_errorbar(aes(ymin=Mean_Slope_DepLength-sqrt(abs(Var_Slope_DepLength)),ymax=Mean_Slope_DepLength+sqrt(abs(Var_Slope_DepLength))), position=dodge, width=.25) +
    facet_wrap(~ Language) +
    geom_bar(stat="identity", position=dodge)
  print(plot)
  plot = ggplot(agg3, aes(x=Head,y=Mean_Slope_DepLength, color=FileName, fill=Real_Distance_Mean)) +
    geom_errorbar(aes(ymin=Mean_Slope_DepLength-sqrt(abs(Var_Slope_DepLength)),ymax=Mean_Slope_DepLength+sqrt(abs(Var_Slope_DepLength))), position=dodge, width=.25) +
    facet_wrap(~ Language) +
    geom_bar(stat="identity", position=dodge)
  print(plot)

  plot = ggplot(agg3, aes(x=Head,y=Mean_Slope_DepLength, color=FileName, fill=RegType)) +
    geom_bar(stat="identity", position=dodge) +
    geom_errorbar(aes(ymin=Mean_Slope_DepLength-sqrt(abs(Var_Slope_DepLength)),ymax=Mean_Slope_DepLength+sqrt(abs(Var_Slope_DepLength))), position=dodge, width=.25) +
    facet_wrap(~ Language) 
  print(plot)
...
agg = agg %>% group_by(Language, RegType, Head) %>% summarise(Mean_Slope_DepLength = weighted.mean(Mean_Slope_DepLength, Count, na.rm=TRUE), Var_Slope_DepLength = weighted.mean(Var_Slope_DepLength, Count, na.rm=TRUE), Real_DH_Mean = weighted.mean(Real_DH_Mean, Count, na.rm=TRUE), Real_Distance_Mean = weighted.mean(Real_Distance_Mean, Count, na.rm=TRUE)) %>% mutate(Slope_DepLength_t = (Mean_Slope_DepLength / sqrt(Var_Slope_DepLength)))

agg = as.data.frame(agg)

#bound = quantile(unique(agg$TotalCount_Head), 1 - 15.0/length(unique(agg$TotalCount_Head)), na.rm=TRUE)
#
#
#agg = agg %>% filter(TotalCount_Head >= bound)


  plot = ggplot(agg, aes(x=Head,y=Mean_Slope_DepLength, fill=Real_DH_Mean, group=paste(Language,RegType))) +
    geom_bar(stat="identity", position=dodge) #+
#    geom_errorbar(aes(ymin=Mean_Slope_DepLength-sqrt(abs(Var_Slope_DepLength)),ymax=Mean_Slope_DepLength+sqrt(abs(Var_Slope_DepLength))), position=dodge, width=.25)
  print(plot)

  plot = ggplot(agg, aes(x=Head,y=Mean_Slope_DepLength, fill=Real_Distance_Mean, group=paste(Language,RegType))) +
    geom_bar(stat="identity", position=dodge) #+
#    geom_errorbar(aes(ymin=Mean_Slope_DepLength-sqrt(abs(Var_Slope_DepLength)),ymax=Mean_Slope_DepLength+sqrt(abs(Var_Slope_DepLength))), position=dodge, width=.25)
  print(plot)

  plot = ggplot(agg, aes(x=Head,y=Mean_Slope_DepLength, fill=RegType, color=Language)) +
    geom_bar(stat="identity", position=dodge) #+
#    geom_errorbar(aes(ymin=Mean_Slope_DepLength-sqrt(abs(Var_Slope_DepLength)),ymax=Mean_Slope_DepLength+sqrt(abs(Var_Slope_DepLength))), position=dodge, width=.25)
  print(plot)
